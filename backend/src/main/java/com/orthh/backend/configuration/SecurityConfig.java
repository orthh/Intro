...
public class SecurityConfig {
    ...
    // 허용되는 URL 목록패턴
    private final String[] allowedUrls = { "/", "/user/**", "/v3/**", "/swagger-ui/**", "/v1/auth/**" };
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(
                        authorize -> authorize
                                .requestMatchers(allowedUrls).permitAll() // 허용된 URL은 모든 요청을 허용
                                .anyRequest().authenticated() // 그 외의 요청은 인증이 필요
                                )
                .formLogin(AbstractHttpConfigurer::disable) // form 로그인을 비활성화
                .sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS)) // 세션을 상태없이 관리
    ...
    public AuthenticationProvider authenticationProvider() {
        ...
        authProvider.setUserDetailsService(jwtUserService.userDetailsService()); // 사용자의 상세 정보를 불러올 때 사용할 서비스를 설정
