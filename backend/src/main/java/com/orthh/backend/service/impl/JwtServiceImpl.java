...
public class JwtServiceImpl implements JwtService {
  ...
  private String generateToken(Map<String, Object> extraClaims, UserDetails userDetails) {
    return Jwts.builder()
        ...
        .setSubject(userDetails.getUsername()) // 클레임과 주제 설정(email)
        .setIssuedAt(new Date(System.currentTimeMillis())) // 발행 시간 설정
        .setExpiration(
            new Date(
                System.currentTimeMillis() + Integer.parseInt(accesstokenExpireTime))) // 만료 시간설정
        .signWith(getSigningKey(), SignatureAlgorithm.HS256)
        .compact(); // 서명 키와 알고리즘을 이용해 서명하고, JWT를 생성
  }
