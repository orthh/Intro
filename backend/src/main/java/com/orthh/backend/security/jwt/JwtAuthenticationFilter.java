...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request,
        ...
        final String authHeader = request.getHeader("Authorization"); // 요청 헤더에서 "Authorization" 값을 가져옵니다.
        final String jwt;
        ...
        if (StringUtils.isNotEmpty(userEmail)
            ...
            UserDetails userDetails = userService.userDetailsService()
                    .loadUserByUsername(userEmail); // 사용자의 상세 정보를 불러옵니다.
            // JWT가 유효한 경우, Security Context에 인증 정보를 설정합니다.
            if (jwtService.isTokenValid(jwt, userDetails)) {
                SecurityContext context = SecurityContextHolder.createEmptyContext(); // 새로운 Security Context를 생성합니다.
                UsernamePasswordAuthenticationToken authToken = new UsernamePasswordAuthenticationToken(
                        userDetails, null, userDetails.getAuthorities()); // 인증 정보를 담은 토큰을 생성합니다.
                authToken.setDetails(new WebAuthenticationDetailsSource().buildDetails(request)); // 토큰에 요청 정보를 설정합니다.
